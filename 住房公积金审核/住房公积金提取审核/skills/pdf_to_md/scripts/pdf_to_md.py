...
MAX_RETRY_WAIT = 60  # cap exponential backoff at 60s


def _ocr_single_page(pdf_bytes: bytes, api_key: str, api_url: str) -> str:
    """Send a single-page PDF (bytes) to DeepSeek-OCR and return cleaned text.

    Retries with exponential backoff on HTTP 429 (rate limit), up to 60s max wait.
    """
    ...
    with httpx.Client(transport=transport, timeout=180.0) as client:
        while True:
            ...
            if resp.status_code == 429:
                wait = min(backoff, MAX_RETRY_WAIT)
                time.sleep(wait)
                backoff = min(backoff * 2, MAX_RETRY_WAIT)
                continue
...
def pdf_to_md(file_path: str) -> dict[str, str]:
    ...
    api_url = os.environ.get("OCR_BASE_URL", DEFAULT_API_URL)

    if not api_key:
        raise ValueError("API Key required for PDF OCR")
    ...
    def _process_page(page_idx: int, pdf_bytes: bytes) -> tuple[int, str]:
        try:
            text = _ocr_single_page(pdf_bytes, api_key, api_url)
...
if __name__ == "__main__":
    ...
    if len(sys.argv) != 2:
        print(f"Usage: python {sys.argv[0]} <pdf_file_path>", file=sys.stderr)
        sys.exit(1)
